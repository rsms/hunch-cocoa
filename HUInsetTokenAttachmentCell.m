...
@interface HUInsetTokenAttachmentCell (BWTACPrivate)
+ (NSImage *)arrowInHighlightedState:(BOOL)isHighlighted;
@end

@implementation HUInsetTokenAttachmentCell
...
+ (void)initialize
{
	...
	blueGradient = [[NSGradient alloc] initWithStartingColor:[NSColor colorWithDeviceWhite:0.95 alpha:1.0]
																							 endingColor:[NSColor colorWithDeviceWhite:0.85 alpha:1.0]];
	...
	blueStrokeGradient = [[NSGradient alloc] initWithStartingColor:[NSColor colorWithDeviceWhite:0.0 alpha:0.0]
																										 endingColor:[NSColor colorWithDeviceWhite:0.0 alpha:0.4]];
	...
	blueInsetGradient = [[NSGradient alloc] initWithStartingColor:[NSColor colorWithDeviceWhite:1.0 alpha:1.0]
																										endingColor:[NSColor colorWithDeviceWhite:1.0 alpha:0.8]];
	...
	[textShadow setShadowOffset:NSMakeSize(0.0, 1.0 / [[NSScreen mainScreen] userSpaceScaleFactor])];
...
#define _subtractXMargin 5.0

- (NSSize)cellSizeForBounds:(NSRect)r {
	NSSize s = [super cellSizeForBounds:r];
	NSFont *font = [self font];
	//s.height = [font capHeight] + ([font ascender] - [font xHeight]) + (-[font descender]); // cap-height  4.0
	s.height = [font capHeight] + 4.0;
	s.height += 5.0; // line-height
	//s.width += _subtractXMargin * 2.0;
	return s;
}

- (NSRect)titleRectForBounds:(NSRect)r1 {
	NSRect r = [super titleRectForBounds:r1];
	r.origin.y += 2.5; // adjust base of text rect
	r.origin.x -= _subtractXMargin;
	r.size.width += _subtractXMargin * 2.0;
	return r;
}

- (NSRect)cellFrameForTextContainer:(id)textContainer proposedLineFragment:(NSRect)lineFragRect glyphPosition:(NSPoint)glyphPosition characterIndex:(unsigned int)charIndex {
	NSRect r = [super cellFrameForTextContainer:textContainer proposedLineFragment:lineFragRect glyphPosition:glyphPosition characterIndex:charIndex];
	r.origin.x -= _subtractXMargin;
	r.size.width -= _subtractXMargin * 2.0;
	return r;
}
